	...
	if(status==statusDB){
		...
		while (j != statusDB.constEnd()) {
		    ...
		    if(seq > status.value(userID).toUInt()){
		    	...
		    	qDebug() << "[GNode::process_status]Sharing message with neighbor: " << message;
...
void GNode::random_send(QByteArray data){
	// as of now we randomly choose between neighbors
	// qDebug() << "Neighbors are:" << neighbor1 << "  " <<neighbor2;
	if(rand()%2 == 0 && neighbor1 >= 0)
		send_messageUDP(data,QHostAddress::LocalHost,neighbor1);
	else{
		if(neighbor2 >= 0)
			send_messageUDP(data,QHostAddress::LocalHost,neighbor2);
	} 
	qDebug() << "[GNode::random_send]Message sent to random neighbor";
}
...
void GNode::received_message2send(QString message){
	...
	if(neighbor1 > 0)
		SerializeSend_message(map_message,QHostAddress::LocalHost,neighbor1);
	if(neighbor2)
		SerializeSend_message(map_message,QHostAddress::LocalHost,neighbor2);
